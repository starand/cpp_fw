...
{
	EIRCOMMAND eircCommand = EIRC_NOCMD;
	
	if( wait ) while( -1 == QFront );
	else 
	{
		if( -1 == QFront ) return (EIRCOMMAND)RC_NONE;
	}
		
	eircCommand = IrCmdQ[ QFront ];

	if( QFront == QEnd ) QFront = QEnd = -1;
	else
	{
		if( QFront == (QMAX-1) ) QFront = 0;
		else QFront++;
	}
	
	return (EIRCOMMAND)eircCommand;
}
